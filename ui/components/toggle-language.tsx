    ...
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          ...
          align="center"
          sideOffset={10}
          className="dark:bg-[#141210] bg-white shadow-[0_3px_15px_rgba(0,0,0,.08)] dark:shadow-[0_3px_30px_rgba(0,0,0,.7)] border border-stone-500/20 rounded-xl min-w-[200px]"
        >
            ...
            {Object.entries(languages).map(([key, language]) => {
              return (
                <DropdownMenu.RadioItem
                  key={key}
                  value={key}
                  className="p-1.5 hover:bg-stone-500/10 focus:bg-stone-500/10 flex px-2 items-center gap-2 outline-none text-sm cursor-pointer rounded-lg"
                >
                  {language.name}
